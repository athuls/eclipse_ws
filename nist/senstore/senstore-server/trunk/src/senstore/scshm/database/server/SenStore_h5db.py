...
# All rights reserved.
# $Id: h5db-tpl.py,v 1.7 2010/06/19 00:13:47 glinden Exp $
import sys
import traceback
import logging
from tables import *
...
class Database:
    ...
    # @return vector data
    def getMappingMatrixArrayData(self, id, i, idxs):
        grp = self.h5.getNode(self.grpMappingMatrix, 'id%d' % id)
        array = self.h5.getNode(grp, 'data')
        sel = list(idxs)
        sel[i] = slice(None,None,1)
        return list(array.__getitem__(tuple(sel)))
    ...
    def setMappingMatrixArrayData(self, id, i, idxs, data):
        grp = self.h5.getNode(self.grpMappingMatrix, 'id%d' % id)
        array = self.h5.getNode(grp, 'data')
        sel = list(idxs)
        sel[i] = slice(None,None,1)
        array.__setitem__(tuple(sel), data)
        array.flush()
    ...
    def getDaqUnitChannelDataSignalData(self, id, tStart, tStep, tEnd, slices):
        ...
        tAxis = self.h5.getNode(grp, 'time')
        # Find timestamps to retrieve data for
        t = tStart
        i = 0
        while True:
        ...
        if len(idxlist) == 0:
                return (0,), [], []
        # Determine shape of result, taking the requested slice into account
        dataShape = list(data.shape)
        dataSlice = [Ellipsis for x in dataShape]
        dataShape[0] = len(idxlist)
        if len(dataShape) == (len(slices) + 1):
            for i in range(1,len(dataShape)):
                s = slices[i-1]
                if s == None:
                    continue
                dataShape[i] = len(range(s.start,s.end,s.step))
                dataSlice[i] = slice(s.start,s.end,s.step)
        # Create array to hold result data
        dOut = numpy.zeros(tuple(dataShape), dtype=numpy.int32)
        tOut = numpy.zeros((len(idxlist),), dtype=numpy.float64)
        # Copy data from HDF5 into array
        for i,idx in enumerate(idxlist):
            tOut[i] = tAxis[idx]
            dataSlice[0] = idx
            dOut[i] = data[tuple(dataSlice)]
    ...
    def getDaqUnitChannelDataSignalDataByIndexRange(self, id, idxStart, idxStep, idxEnd, slices):
        ...
        # Build data slice
        #
        dataSlice = [Ellipsis for x in list(data.shape)]
        # Time axis slice
        dataSlice[0] = slice(idxStart,idxEnd,idxStep)
        # Other dimensions
        if len(tOut) == (len(slices) + 1):
            for i in range(1,len(tOut)):
                s = slices[i-1]
                if s == None:
                    continue
                dataSlice[i] = slice(s.start,s.end,s.step)
        dOut = data[tuple(dataSlice)]
        # Compute total number of elements
        n = reduce(int.__mul__, dOut.shape)
        return dOut.shape, dOut.reshape((n,)).tolist(), tOut.tolist()
    ...
    def getAccelMeasSignalData(self, id, tStart, tStep, tEnd, slices):
        ...
        if len(dataShape) == (len(slices) + 1):
            for i in range(1,len(dataShape)):
                ...
                dataSlice[i] = slice(s.start,s.end,s.step)
        # Create array to hold result data
        dOut = numpy.zeros(tuple(dataShape), dtype=numpy.float32)
        tOut = numpy.zeros((len(idxlist),), dtype=numpy.float64)
        # Copy data from HDF5 into array
        for i,idx in enumerate(idxlist):
            tOut[i] = tAxis[idx]
