...
# $Id: dbdef.py,v 1.2 2010/06/16 19:01:14 glinden Exp $
import logging
...
class Vars(dict):
    ...
    ## Adds a new group (list) to the variable set.
    #
    # @param key   variable name
    # @param value list (optional; default is empty list)
    # @return list
...
def getSyntax():
    ...
    refDef = typeName + Literal('*').hide() + varName + Literal(';').hide()
...
def read(defFile, basename='SenStore'):
    ...
    dbVars.pgdbName = basename.lower()
