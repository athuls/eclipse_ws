...
class EntityInfoTag {
  public static function getEntityInfoPageName($dataSourceName, $entity) {
    return 'MD_ENTITY_' . $dataSourceName . '_' . $entity;
  }
  
  private static function setUpEntityInfoPage($dataSourceName, $entity, $parser) {
    ...
    $entityInfoPageName = self::getEntityInfoPageName($dataSourceName, $entity);
    ...
    ModelDoc\MediaWiki::replacePageText($entityInfoPageName,
          ...
          . '[['
          . Versioning\EntityVersionManager::getEntityVersionPageName($dataSourceName, $entity)
          . '|REVISION HISTORY]] '
          . "[[MD_REFRESH_$dataSourceName_$entity|REFRESH]]"
          );
  ...
  public function mediaWikiHook($input, $params, $parser) {
    ...
    $tableListHTML = "<h3>Table: " . $parser->recursiveTagParse("[[$entityInfoPageName|$entity]]") . '</h3>'
    ...
    foreach ($constraints as $constraint) {
      ...
      if ($constraint->constraintType == 'FOREIGN KEY') {
        $tableListHTML .= $parser->recursiveTagParse('[['
            . self::getEntityInfoPageName($dataSourceName,
                $constraint->tableReferenced . '|' . $constraint->tableReferenced . ']]'));
      }
