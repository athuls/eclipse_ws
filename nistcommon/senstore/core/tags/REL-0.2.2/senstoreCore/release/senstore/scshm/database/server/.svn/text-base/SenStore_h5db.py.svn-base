...
# $Id: h5db-tpl.py,v 1.15 2011/05/10 18:17:14 glinden Exp $
import sys
import traceback
import logging
from tables import *
...
class Database:
    ...
    def __init__(self, filename, title = ''):
        ...
        self.h5 = openFile(filename, mode = "a", title = title, filters = filters)
        #
    ...
    def getDaqUnitChannelDataTimeAxisInfo(self, id, tList):
        ...
        if not nodeExists(grp, 'time'):
            # There is no signal; return empty result
            return [],[]
        tAxis = self.h5.getNode(grp, 'time')
        return findNextClosestList(tAxis,tList)
    ...
    # @param idxStop    end index (not included in data)
    # @return vector with time stamps \[s]
    def getDaqUnitChannelDataTimeAxisByIndexRange(self, id, idxStart, idxStep, idxStop):
        # TODO: faster algorithm for searching
        grp = self.h5.getNode(self.grpDaqUnitChannelData, 'id%d' % id)
        if not nodeExists(grp, 'time'):
            # There is no signal; return empty result
            return []
        tAxis = self.h5.getNode(grp, 'time')
        return tAxis[idxStart:idxStop:idxStep]
    
    ## Gets the signal data.
    #
    # The returned signal data will have time stamps tStart <= t <=
    # tStop, with no two time stamps closer than tDelta.
    #
    # @param id      DaqUnitChannelData object ID
    ...
    # @param slices  list of slices for each dimension (empty is all)
    # @return (shape, data, t) 
    def getDaqUnitChannelDataSignalData(self, id, tStart, tStep, tStop, slices):
        # TODO: faster algorithm for searching
        idxlist = []
        grp = self.h5.getNode(self.grpDaqUnitChannelData, 'id%d' % id)
        ...
        t = tStart
        idx1 = 0
        idx2 = findNextClosest(tAxis, tStop)
        if idx2 == None:
            idx2 = -1
        while True:
            idx1 = findNextClosest(tAxis, t, idx1, idx2)
            if idx1 == None:
                break
            if tStep >= 0.0:
                t = tAxis[idx1] + tStep
    ...
    # @param slices    list of slices for each dimension (empty is all)
    # @return (shape, data, t) 
    def getDaqUnitChannelDataSignalDataByIndexRange(self, id, idxStart, idxStep, idxStop, slices):
        grp = self.h5.getNode(self.grpDaqUnitChannelData, 'id%d' % id)
        data = self.h5.getNode(grp, 'data')
        tAxis = self.h5.getNode(grp, 'time')
        tOut = tAxis[idxStart:idxStop:idxStep]
        if len(tOut) == 0:
            return (0,), [], []
        #
        # Build data slice
        ...
        for i,s in enumerate(dataSlices):
            if i == 0:
                # Time axis slice
                dataSlices[0] = slice(idxStart,idxStop,idxStep)
                continue
            if s == Ellipsis:
                # PyTables does not support more than one Ellipsis
                dataSlices[i] = slice(0,data.shape[i],1)
            else:
                dataSlices[i] = slice(s.start,s.stop,s.step)
        dOut = data[tuple(dataSlices)]
        # Compute total number of elements
        n = reduce(int.__mul__, dOut.shape)
    ...
    # @param idxStop    end index (not included in data)
    # @return vector with time stamps \[s]
    def getAccelMeasTimeAxisByIndexRange(self, id, idxStart, idxStep, idxStop):
        # TODO: faster algorithm for searching
        grp = self.h5.getNode(self.grpAccelMeas, 'id%d' % id)
        if not nodeExists(grp, 'time'):
            # There is no signal; return empty result
    ...
    # @param slices  list of slices for each dimension (empty is all)
    # @return (shape, data, t) 
    def getAccelMeasSignalData(self, id, tStart, tStep, tStop, slices):
        # TODO: faster algorithm for searching
        idxlist = []
        grp = self.h5.getNode(self.grpAccelMeas, 'id%d' % id)
        data = self.h5.getNode(grp, 'data')
    ...
    # @param slices    list of slices for each dimension (empty is all)
    # @return (shape, data, t) 
    def getAccelMeasSignalDataByIndexRange(self, id, idxStart, idxStep, idxStop, slices):
        grp = self.h5.getNode(self.grpAccelMeas, 'id%d' % id)
        data = self.h5.getNode(grp, 'data')
        tAxis = self.h5.getNode(grp, 'time')
        tOut = tAxis[idxStart:idxStop:idxStep]
