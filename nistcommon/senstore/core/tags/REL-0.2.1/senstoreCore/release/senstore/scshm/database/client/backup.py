...
# $Id: backup-tpl.py,v 1.14 2011/05/10 18:17:14 glinden Exp $
import sys
import traceback
import logging
import ConfigParser
...
class BackupFile:
    ...
    def __init__(self, filename, mode = 'r', title = ''):
        ...
        self.h5 = openFile(filename, mode = mode, title = title, filters = filters)
        #
        ...
        if mode == 'w':
            ...
            row['label'] = 'METRICTON'
            row.append()
            self.UnitEnumList.append('UnitMETRICTON')
            row = self.tblUnit.row
            row['label'] = 'NEWTON'
            row.append()
            self.UnitEnumList.append('UnitNEWTON')
            row = self.tblUnit.row
            row['label'] = 'KILONEWTON'
            row.append()
            self.UnitEnumList.append('UnitKILONEWTON')
            row = self.tblUnit.row
            row['label'] = 'INCH'
            row.append()
            self.UnitEnumList.append('UnitINCH')
            row = self.tblUnit.row
            row['label'] = 'FOOT'
            row.append()
...
def backup(argv, filename):
    ...
    while i < n:
        ...
        for j,fields in enumerate(fieldsList):
            ...
            if len(dims)==0:
                logging.warn('Skipping MappingMatrix[oid=%d] array data (empty)' % id)
                continue
    ...
    while i < n:
        ...
        for j,fields in enumerate(fieldsList):
            ...
            if len(dims)==0:
                logging.warn('Skipping DaqUnitChannelData[oid=%d] signal data (empty)' % id)
                continue
            ...
            while idxStart < dims[0]:
                idxStop = idxStart + idxStep
                if idxStop > dims[0]:
                    idxStop = dims[0]+1
                result = prx.getSignalDataByIndexRange(idxStart, 1, idxStop, [])
    ...
    while i < n:
        ...
        for j,fields in enumerate(fieldsList):
            ...
            dims = prx.getSignalSize()
            if len(dims)==0:
                logging.warn('Skipping AccelMeas[oid=%d] signal data (empty)' % id)
                continue
...
# @param argv      command-line arguments (typically sys.argv)
# @param filename  HDF5 backup file name
def restore(argv, filename):
    f = BackupFile(filename, 'r')
    # Connect to server with an exclusive access session
    # (this blocks the server for other users!)
    client = SenStoreClient(argv, 'SenStore Restore client', exclusive=True)
    mngr = client.getManager()
    logging.info('Connected to SenStore')
    ...
    if nodeExists(f.h5.root.objects, 'MappingMatrix'):
        ...
        for row in tbl:
            ...
            if not data or len(data.shape) == 0:
                logging.warn('Skipping MappingMatrix[oid=%d] array data (empty)' % id)
                continue
            dims = data.shape
    ...
    if nodeExists(f.h5.root.objects, 'DaqUnitChannelData'):
        ...
        for row in tbl:
            ...
            while idxStart < dims[0]:
                ...
                if idxStop > dims[0]:
                    idxStop = dims[0]+1
                dataChunk = data[idxStart:idxStop].flatten().tolist()
                tChunk = t[idxStart:idxStop].tolist()
